...
void setup()
{
 ...
 void loop()
 {
   ...
   while (BT.available())
   {
     delay(5); //delay to stabilize things
     x = BT.read();
   }
   Serial.println("x:");
   Serial.println(x);
   ...
   if((abs(movec)>1000)&&(abs(turn)>1000))
   {
     movec = 0;
     turn = 0;
   }
   ...
   if(x=='u')//1 for going up
   {
     x='0';
     digitalWrite(dir_ms_1,HIGH);
     digitalWrite(dir_ms_2,LOW);
   }
   else if(x=='d')// 2 for going down
   {
     x='0';
